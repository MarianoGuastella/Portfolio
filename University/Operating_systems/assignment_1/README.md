...
* Ejecutando `make ourtests` se podran compilar los tests que utilizamos para
  parte de experimentacion del informe, los mismos se ejecutan con los
  siguientes parametros:
  ```
  ./tests <func_lectura> <threads_lectura> <func_maximo> <threads_maximo> <archivo1> [<archivo2>...]
  ```
  Donde:
    func_lectura (string): "cargarArchivo" o "cargarMultiplesArchivos"
    threads_lectura (int): cantidad de threads lectura
    func_maximo (string): "maximo" o "maximoParalelo"
    threads_maximo (int): cantidad de threads maximo

    Al usar "maximoParalelo" con 0 threads, se ejecutara variando threads de 1 a 29.
